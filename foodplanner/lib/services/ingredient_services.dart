...
Future<Ingredient> fetchIngredient(http.Client client, AuthProvider authProvider, int id) async {
...
Future<List<Ingredient>> fetchIngredientsByUserID(http.Client client, AuthProvider authProvider) async {
  final jwtToken = await authProvider.retrieveToken();
  // Make a GET request to the API to retrieve ingredients by user ID.
  final response =
      await client.get(Uri.parse('${ApiConfig.baseUrl}/api/Ingredients/GetAllByUser'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
...
Future<http.Response> createIngredient(http.Client client, AuthProvider authProvider, String name,  int? imageRef) async {
...
Future<http.Response> deleteIngredient(http.Client client, AuthProvider authProvider, int id) async {
  final jwtToken = await authProvider.retrieveToken();
