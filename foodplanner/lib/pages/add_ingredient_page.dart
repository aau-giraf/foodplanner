...
class AddIngredientPage extends StatefulWidget {
  ...
  final ValueChanged<List<Ingredient>>
      onIngredientsUpdated; // Callback to the method which modifies the list of existing ingredients
  final VoidCallback
      onCamera; // Callback to change the shown page through "add_meal_page.dart"
  final ValueSetter
      onIngredientAdded; // Callback to handle what to do once a new ingredient is added.
  final Client client;
  ...
  _AddIngredientPageState createState() =>
      _AddIngredientPageState(); // Create state for the page.
}
...
class _AddIngredientPageState extends State<AddIngredientPage> {
  ...
  void dispose() {
    _searchBarController
        .dispose(); // Dispose of the text controller to free up resources.
    super.dispose();
  }
  ...
  Widget _buildAddIngredientPage(
      ...
      children: [
        ...
        Padding(
          padding: EdgeInsets.only(
              top: 5,
              right: 16,
              left: 16), // Adding padding around the search bar.
          child: TextField(
            controller:
                _searchBarController, // Connect the controller to the text field.
            ...
            decoration: InputDecoration(
              ...
              fillColor: AppColors
                  .textFieldBackground, // Background color of text field.
            ),
            onChanged: (text) {
              // Updates the list when changes in the search field happen.
              setState(() {
                sortedIngredients = widget.ingredients.where((ingredient) {
                  return ingredient.name.toLowerCase().contains(
                      text.toLowerCase()); // Filter ingredients by name.
                }).toList();
              });
            },
          ),
        ...
        ), // Divider below the search bar.

        // The list of available ingredients.
        Expanded(
            // Allows the list of ingredients to take remaining available space.
            child: ListView.separated(
          // Creates a list view with separators between items.
          itemCount: sortedIngredients
              .length, // Number of items in the soreted ingredient list.
          itemBuilder: (BuildContext context, int index) {
            // Builds each item in the list.
            return TextButton(
                // A button for each ingredient.
                onPressed: () {
                  // Action when the button is pressed.
                  if (sortedIngredients[index].foodImageId == null) {
                    // Checks if the ingredient has an image.
                    showCupertinoDialog(
                      ...
                      builder: (BuildContext context) => CupertinoAlertDialog(
                        // Build the dialog.
                        title: Text(
                            'Der er ikke et billede til denne ingrediens, tilføj dette nu.'), // Title of the dialog.
                        actions: <CupertinoDialogAction>[
                          ...
                          CupertinoDialogAction(
                            ...
                            onPressed: () async {
                              ...
                              widget
                                  .onCamera(); // Calls the passed callback to navigate to the camera page.
                            },
                ...
                child: Align(
                  // Aligns the child within the button.
                  alignment: Alignment.centerLeft, // Aligns text to the left.
                  child: Text(sortedIngredients[index].name,
                      style: AppTextStyles
                          .standard), // Displays the ingredient name with specified text style.
                ));
          },
          separatorBuilder: (BuildContext context, int index) {
            // Builds a separator between items.
            return Divider(
              color: AppColors.textFieldBorder,
