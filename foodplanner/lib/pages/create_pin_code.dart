...
class CreatePinCode extends StatefulWidget {
  ...
  @override
  CreatePinCodeState createState() => CreatePinCodeState();
}

class CreatePinCodeState extends State<CreatePinCode> {
  List<int> pin = [];
  ...
  void handlePin(int number) async {
    ...
    if (pin.length == 4) {
      ...
      var error = await CreatePinCode.pinService.postPin(25, pin);
      /* if (!error) {
        context.go('/');
      } else {
        print("Error: $error");
